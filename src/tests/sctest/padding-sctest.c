/*
 * Original source: https://github.com/tomato42/marvin-toolkit/blob/master/example/openssl/time_decrypt.c
 */
...
int main(int argc, char *argv[]) {
    ...
    while ((opt = getopt(argc, argv, "i:o:k:n:h")) != -1 ) {
        switch (opt) {
            ...
            case 'o':
                ...
                break;
            case 'n':
                sscanf(optarg, "%d", &ciphertext_len);
                fprintf(stdout, "Running with length: %d\n", ciphertext_len);
                break;
            case 'h':
                help(argv[0]);
                exit(0);
    ...
    if (!in_file_name || !out_file_name || !ciphertext_len) {
    ...
    fprintf(stderr, "Stripping padding...\n");
    ...
    while ((r_ret = read(in_fd, ciphertext, ciphertext_len)) > 0) {
        ...
        sc_pkcs1_strip_02_padding_constant_time(NULL, ciphertext_len, ciphertext, ciphertext_len, plaintext, &plaintext_len);
