...
The `fuzz_piv` target allows testing operations of `piv-tool`. What operation is tested depends of first byte of the fuzzing input:
...
### The pkcs15-tool fuzzer

The `fuzz_pkcs15` target allows testing operations of `pkcs15-tool`. The options are taken from fuzzing input, it is parsed as\
