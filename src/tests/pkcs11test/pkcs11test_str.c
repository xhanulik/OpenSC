...
int
lookup_flags(CK_ULONG value, char result[])
{
	size_t index = 0;
	CK_ULONG size = sizeof(ck_flags) / SZ_SPECS;
	for (CK_ULONG i = 0; i < size; i++) {
		if (ck_flags[i].type & value) {
			if (index == 0) {
				strcpy(result, ck_flags[i].name);
			} else {
				result[index] = '|';
				strcpy((result + 1), ck_flags[i].name);
			}
			return PKCS11TEST_SUCCESS;
		}
	}
